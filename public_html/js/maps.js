...
function pegaRep(latitude, longitude, raio){ //Essa funcao nao eh chamada apenas na inicializacao/ Seu raio eh em km
	var xmlhttp;
	var result;
	var url = "/republicas.php?lat="+latitude+"&lng="+longitude+"&r="+raio;
...
function marcaRep(valores){
	...
	for (var i = 0; i<valores.length; i++) {
		...
		markers[i] =new google.maps.Marker({
			...
			map:map,
			title:name,
			icon:"img/mapa-marcador-50-sombra1.png" 
		});
...
function pesquisar(){ //Funcao principal - chama as demais		
    ...
    //document.getElementById('status').innerHTML='<b> realizada! </b>'+encontradas+" festas encontradas.";
